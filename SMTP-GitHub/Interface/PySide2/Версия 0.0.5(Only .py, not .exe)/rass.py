...
def attach_file_save(attach_filenames,filenames):
    global file_count
    file_count = len(filenames)
    filename_t = attach_filenames
    print(filename_t)
    return filename_t
...
def send_email(i, name, filename_t):
    ...
    with open(filename_t, 'rb') as attachment:
        part = MIMEApplication(attachment.read(), Name=os.path.basename(filename_t))
        part['Content-Disposition'] = f'attachment; filename="{Header(os.path.basename(filename_t), "utf-8").encode()}"'
        msg.attach(part)
